...
class GantryFeatureStyleDeclaration extends GantryFeature {
	...
	function init() {
		...
		//Load and initialize Nawalas GantryHelper class
		...
		$lessVariables = array(
			...
			'iconfont'    => $gantry->get('iconfont',    'font-awesome'),
			'layout-mode' => $gantry->get('layout-mode', '100responsive')
		);
		$gantryHelper->setVariables($lessVariables);

		$gantryHelper->addLess( 'global.less', 'master.css', 8 );
		...
		if ( $gantry->get('layout-mode') == "100responsive" ) {
//			$gantryHelper->addLess('mediaqueries.less', 'mediaqueries.css');
		}
		if ( $gantry->get('layout-mode') == "1200responsive" ) {
			$gantryHelper->addLess('mediaqueries.less', 'mediaqueries.css');
		}
