...
//				JFile::delete( $fileOut );
	...
	public function importLess( $files, $compiledName = false, $priority = self::DEFAULT_STYLE_PRIORITY, $lessVariables = false, $compress = false, $template_files_override = false )
	{
		...
		if ( $compress ) {
			require_once(NAWALA_BASEPATH_FILESYSTEM . '/libraries/nawala/compiler/cssmin.inc.php');
		}
...
//					JFile::delete( $fileOut );
		...
		if ( $create ) {
			if ( $compress ) {
				...
				$cssBuffer = CssMin::minify( $lessc->compile( $content ) );
			} else {
		...
		if ( $compress ) {
			// CssMin::minify( file_get_contents("path/to/source.css") );
			$buffer = CssMin::minify( $lessc->compile( $content ) );
		} else {
			$buffer = $lessc->compile( $content );
		}
